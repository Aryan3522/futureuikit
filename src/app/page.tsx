...
const AnimatedTerminal = () => {
  const [lines, setLines] = React.useState<{type: string, text: string}[]>([]);
  ...
  useEffect(() => {
    ...
    if (step.type === "command") {
      if (typedCommand.length < step.text.length) {
        const timeout = setTimeout(() => {
          setTypedCommand(step.text.slice(0, typedCommand.length + 1));
        }, Math.random() * 40 + 30);
        return () => clearTimeout(timeout);
      } else {
        const timeout = setTimeout(() => {
  ...
  const renderOutputLine = (text: string) => {
    if (text.startsWith("✓")) {
      return <><span className="text-[#00ffcc] mr-2">✓</span><span className="text-[#a1a1aa]">{text.substring(1)}</span></>;
    }
    if (text.startsWith("▲")) {
      return <><span className="text-white font-bold mr-2">▲</span><span className="text-[#a1a1aa]">{text.substring(1)}</span></>;
    }
    ...
    >
      {lines.map((line, i) => (
        <div key={i} className="flex gap-4 w-full">
          {line.type === "command" && <span className="text-[#8b5cf6] font-bold shrink-0">$</span>}
          <div className={cn("whitespace-pre-wrap leading-relaxed", line.type === "command" ? "text-white" : "")}>
            {line.type === "command" ? line.text : renderOutputLine(line.text)}
          </div>
        </div>
      ))}
      {currentStep < TERMINAL_STEPS.length && TERMINAL_STEPS[currentStep].type === "command" && (
        <div className="flex gap-4 w-full">
          <span className="text-[#8b5cf6] font-bold shrink-0">$</span>
          <span className="text-white">
            {typedCommand}<span className="inline-block w-2 h-4 bg-[#8b5cf6] animate-pulse ml-1 align-middle" />
          </span>
        </div>
      )}
    </div>
...
export default function Home() {
  ...
  // Removed mouse tracking as per user request to allow OrbitControls on the 3D model instead.
    ...
    <div className="min-h-screen bg-background text-foreground font-body-md overflow-x-hidden selection:bg-secondary/30 relative">
      ...
      <main className="relative z-10">
        ...
        <section className="relative min-h-[90vh] flex flex-col items-center justify-center pt-32 pb-16 px-6">
          ...
          >
            <div className="w-[80vw] h-[80vw] max-w-[800px] max-h-[800px] bg-secondary/20 rounded-full blur-[150px] opacity-30 mix-blend-screen" />
          </motion.div>
        ...
        <section className="py-32 px-6 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <GlassPanel variant="heavy" className="md:col-span-8 p-10 flex flex-col justify-between group overflow-hidden relative min-h-[400px]">
            ...
            <GlassPanel variant="heavy" className="md:col-span-4 p-10 space-y-6 hover:border-secondary/50 transition-colors duration-500 min-h-[400px] flex flex-col justify-center">
            ...
            <GlassPanel variant="heavy" className="md:col-span-8 p-10 flex flex-col md:flex-row items-center gap-10 min-h-[400px]">
        ...
        {/* CODE PREVIEW */}
        <section className="py-32 px-6 bg-foreground/[0.02] border-y border-border relative">
          <div className="max-w-5xl mx-auto">
            ...
            <GlassPanel variant="heavy" className="p-1 h-[300px]">
              <AnimatedTerminal />
            </GlassPanel>
          </div>
        </section>
        ...
        <section className="py-40 px-6 text-center relative overflow-hidden">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-secondary/10 blur-[150px] rounded-full pointer-events-none mix-blend-screen"></div>
